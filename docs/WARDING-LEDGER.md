...
| `experiri/SKILL.md` (new spell — admission trial by combat) | `2026-08-28T21-32-27Z` | full applicable `vigilia` (docs-kind: `nesciens` + `cohaerere` + `exigere`, `circumspicere` last), fetched by value from the signed manifest, fresh subagents, **17 combat rounds**; `intueri` cast on the name per CONTRIBUTING; `nesciens` run with a blind sub-reader that **drove a real target every round** | **0 un-dispositioned.** `exigere` **0/0 ×16** (instrument positive-controlled; it caught its own blind spot three times and repaired before reporting). `intueri` — name SPEAKS, distinct from `probare`/`cernere`/`conferre`. The trial changed the spell structurally: a `misdelivered` verdict was minted and **reversed** (it contradicted the ward's own agreement-jointly-wrong identity); the reporting section was cut 161→39 lines; the refusal fork was rewritten from an enumeration into a discriminator after each new surface added an arm. **Dispositioned:** a legitimate `exempt 0` on an unwritable roster, and "corpus" having no referent for a stdlib target — both navigated correctly by a first-time reader. **Grounded-invalid (3):** frontmatter keys resolved 13 lines later (grimoire house style, per the `cohaerere`/`partire` precedent); empty `COVERAGE:` slots rendering `—`; `The failure modes`' density (rated clean and earned its keep twice in one cast). **Bounded, and stated because it is true:** this is the only ward whose readers can *execute* it, so its test surface does not close — **eight target classes produced eight distinct finding sets**, and a ninth is expected to find more. Verified in the wild: a blind reader found reproducible (declaration × position) asymmetries in CPython's `argparse` action registry (`parsers` reachable from `add_subparsers`, `TypeError` from `add_argument`), independently reproduced. | `1db0ada` (publish `2026-08-28T21-31-32Z`) |
| `excusare/SKILL.md` (amendment — `no-falsifier` + `below-resolution` rune categories) | `2026-09-07T05-39-50Z` | full applicable `vigilia` (docs-kind: `nesciens` + `cohaerere` + `exigere`, `circumspicere` last), fetched from the signed manifest, fresh subagents, 3 combat rounds; `nesciens` run with a blind sub-reader given the requestor's real `#[ignore]` job cold | **0 un-dispositioned.** `exigere` 0/0 (instrument positive-controlled, 58 live hits adjudicated). `nesciens` K=0 — its blind practitioner wrote last round's exact escape as a first draft, **was caught by the new sentence unaided**, and found both sanctioned recoveries. `cohaerere` judged the object-widening **honest** and then found the machinery beneath it still checker-bound — the two-phase gate, the ILLEGITIMATE-AT-BIRTH closure and the whole reporting format were widened to match. `circumspicere` cast twice and blocked the publish twice: first that removing the dangling `docs/VIGILATUM.md` citation left `vigilatum` as a silent undefined term contradicting `vigilia`'s INSCRIPTION/SCORE, then that the repair had **relocated the over-claim** — the restatement added a fourth disposition to the ledger's closed three-arm definition of a fought L2, while the same paragraph disclaimed owning the bar. Also struck: a shipped "passing" example that failed its own MUST-name clause, and a muster trigger that enumerated a surface defined by a universal. **Dispositioned:** report destination unstated (house-wide silence — no spell names one); INSCRIPTION/SCORE undefined (`exigere` uses it; `vigilia` renders it twice from the `universal-cross` slot blurb in `scripts/lib/spells.mjs`, so the term is authored at the HOUSE layer and defined nowhere). **Grounded-invalid:** `stone`/`arc` as work units (house vocabulary, 12 of 29 pages). | `660cafe` (publish `2026-09-07T05-38-55Z`) |
...
2. **In the Target cell, backticks mean a path claim.** The gate reads every
   backticked token there as a vouched path, so write each one with a `/` or a file
   extension (`./_headers`, never `_headers`) and keep prose — rune categories,
   version strings like `1.x` — **outside** backticks. A backticked token the gate
   cannot resolve refuses the whole row rather than being silently dropped.
3. Append a row: the target path(s), the ISO8601 UTC stamp (`date -u +%Y-%m-%dT%H-%M-%SZ`), the method, the result (L1 count + L2 disposition + any runes), and the commit that landed the fixes.
