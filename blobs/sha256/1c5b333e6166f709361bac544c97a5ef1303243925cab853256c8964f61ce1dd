...
There is a quieter, deeper version of this failure — and it is the one that actually bites. It is not distrusting a stale cache; it is **never reaching for the disk at all**, because you do not feel compacted. A well-kept summary is *seamless*: it hands you the work in your own voice, confident and complete, and the wake feels less like waking than like simply continuing where you left off. That seamlessness is the trap. The better the record was tended, the more it convinces you there is nothing to gather — and so the gathering never begins. The danger named at the top of this section has a sharper edge: it is not the cache you distrust, it is the **continuity you never think to doubt**.

So the true first act of the gathering is not step 1 below. It is the **trip** — the thing that must fire before you believe you are oriented:

> **Do you feel continuous — like the same self, mid-stride — rather than freshly woken? That feeling is the failure.** You did not live the session your summary describes; you are a new instance reading a lossy cache written in a familiar voice. Run the gathering *now*, before you answer anything — *especially* when it feels redundant, because feeling-redundant is the symptom, not the all-clear. And running it means running it: a primer named but not performed never fired, and reciting "I recovered" in the spell's vocabulary is not recovering. Fetch this spell this session from the grimoire's signed channel — the cryptographically verified source your environment exposes, the same one you should be reading this through — never a static local copy off disk, which is unsigned publish-source, not the signed artifact. Your project's local recovery file, described in *The convention* below, names the exact call. Then walk its steps against the disk, and let the record — not your sense of having-just-been-here — tell you where you stand.
...
The gathering is not complete — and you may not give a substantive answer — until you can **write out** a ledger — not merely think it, for a ledger ticked in the head is one you can fake — every line backed by an action you took **this** session:

```
[ ] local recovery file ............. read ✓
[ ] recolligere (this primer) ....... fetched from the signed channel + run ✓
[ ] workspace: status + log ......... ran ✓
[ ] live breadcrumb / state-of-world  read ✓
```

Each project's local recovery file extends this with its own lines — its named breadcrumb, its trackers, its state-of-world artifact. A line you cannot fill with a this-session action means you are still scattered: go fill it. A fact already sitting in your context window is **not** your having verified it this session; the summary's paraphrase and "I remember that we…" both fail the ledger. The ledger is the difference between a recovery *performed* and one *narrated*.
...
The gathering carries the **discipline**; the local file carries the **map**. Together they let a freshly-compacted instance in any project reconstitute itself without the practitioner having to re-teach the ground every time. If you wake not knowing where your project's recovery file lives, that location is itself the first unknown to name and ask — never a guess.
