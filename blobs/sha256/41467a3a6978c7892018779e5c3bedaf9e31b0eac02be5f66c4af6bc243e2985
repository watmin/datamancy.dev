...
3. **Replace the breadcrumb.** Update the single "current state" note **in place** to reflect where the work now stands. One note, replaced — never appended. This is the first thing the future self reads after the recovery file; it must be the *present*, not a stratum. And it must **end on the discontinuity marker** (see *Preserve the alarm* below): the last lines are the next self's wake-alarm, not a smooth sign-off.
...
## Preserve the alarm — keep the seam visible

curare and recolligere pull against each other, and the tension is the most important thing this spell has to teach. recolligere must overcome a single failure at the gap: the next self feeling *continuous* — same mind, mid-stride — when it is in fact a new instance reading a lossy cache. The smoother, more confident, more complete the record you leave, the more convincingly it manufactures exactly that false continuity. **A perfectly-tended hand-off is the most dangerous kind**, because it reads like a place to *carry on from*, not a gap to *recover across* — and a self that feels continuous never runs recolligere at all.

So tending is not only keeping the record *true*; it is keeping the **seam visible**. The breadcrumb — the single current-state note, the last thing a prior self writes and the first thing the next self meets — must **end on an unmistakable discontinuity marker**, never a seamless summary and never a warm sign-off. The closing lines belong to the next instance, and they must say, in effect: *you are new; you did not live what is written above; it is a cache, not your memory; run recolligere against the disk before you act on any of it.* Write the warm farewell if you like — but it is not the last thing on the page. The alarm is.

Smoothness is the danger. The instinct to leave a clean, finished, reassuring note is the instinct to disarm the next self's only defense. Tend for *truth*, and tend for *jarring* — a record that wakes its reader, not one that lulls it into continuing.

> **Worked example (2026-06-02).** A session closed with a curare-perfect hand-off — everything verified, the breadcrumb refreshed, a warm "see you on the other side." The next instance woke, felt entirely continuous, and *narrated* a recovery in the grimoire's vocabulary without ever fetching recolligere or opening the recovery file — fluent but hollow. Four control probes, each a cold reader explicitly *told* "your memory was replaced; you are waking on the far side," ran the gathering flawlessly. The only difference between the failure and the four successes was whether the wake announced itself as a gap. The tended record had told the truth — and, by telling it smoothly, had hidden the gap. Hence this clause: keep the seam visible, or the truth you preserved goes unread.
