...
Every line of code has a caller. Service consumers, service implementers, framework users, library users — each occupies a vantage. The caller's vantage IS the interface. Anything else is mechanism.
...
- A unit test stands where another internal caller stands.
...
(send req-tx
  (HologramCacheService::Request::Get k reply-tx))
;; Then manually unwraps the Result<Option<T>, ThreadDiedError> chain.
```

This test verifies the wire protocol. Wire-protocol pedagogy lives in the consumer's substrate-primitive reference file (e.g., a service-template that documents the implementer's vantage). A test in a consumer crate should call the consumer surface, not the wire under it.

### Tests with patterns the documentation wouldn't recommend

If the test's call shape doesn't appear in the worked examples in the consumer's USER-GUIDE / README / doc-comments — and a worked example for the same use case exists — the test is at the wrong layer.
...
## What vocare does NOT flag (architectural exemptions)

These are structurally obvious — the file location or surrounding cross-reference is the proof. No rune needed.

- Tests in **substrate-primitive reference files** — files that exist specifically to document HOW to implement a substrate primitive (audience is the service implementer using primitives, not the consumer using the assembled service). The implementer's vantage IS the canonical vantage at these sites.
- **Unit tests in the host-language source tree** (Rust `#[test]` functions in `src/*.rs`, etc.). Their callers are other modules in the same language. The test's vantage matches its actual caller's vantage.
- **Tests that mirror worked examples** in USER-GUIDE / README / doc-comments. The example is the recommended pattern; the test that mirrors it confirms the recommendation. The cross-reference to the example IS the consumer-vantage proof.
...
1. **Identify the caller.** Who calls this code in production? Consumer, implementer, internal caller — pick one.
...
## The rune (site-specific exemptions)

Where the architectural exemptions above cover whole files, the rune
covers a single test inside an otherwise-flaggable file. The test
intentionally tests from a non-consumer vantage — substrate reference,
...
- `substrate-primitive-reference` — test exists to document the substrate primitive itself. The implementer's vantage is the canonical vantage at this site; consumer-vantage doesn't apply.
...
Placement: on the line immediately preceding the test's body
expression — wherever the host language allows a comment to
attach to a test definition.
...
exemption in its report.

## The principle behind the spell

The interface IS the boundary the caller respects. A test that reaches past the boundary tests the implementation, not the contract. The next reader who lifts that pattern learns that the boundary is optional — and the discipline degrades. Vocare defends the boundary by finding where the test forgot whose vantage it was supposed to honor. The four-questions (see grimoire README) apply to every "what would the consumer write?" judgment the practitioner makes.
