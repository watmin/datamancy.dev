...
function render(members) {
  ...
  const lines = [
    ...
    ...rosterRows(members),
    "",
    `Not all spells apply to every target. Vigilia casts only those whose discipline matches the target's **kind**. The **universal code set** (${universalCode}) is cast on every code target; the conditional code wards (${conditional}) join when their own triggers fire — each trigger says on what: the file's contents, the environment it must run in, another ward's finding, or the practitioner's judgment about the target; the kind-scoped wards join when the target matches their kind. Beyond those, **${crossKind}** runs on every target regardless of kind — not just code (its row names where). circumspicere is always cast, and always last. The selection rule below names, per kind, exactly which wards muster.`,
    "",
    "## The cast mechanic — reach the MCP, or embed",
    "",
    "The caster fetches each inward spell's `SKILL.md` from the grimoire once — the origin serves the bytes and the `datamancy` npm adapter, which pins the public key, SHA-256-verifies them against the signed manifest — and **embeds the full text verbatim** into that spell's subagent prompt, alongside the named target. The subagent applies its discipline from what it was handed.",
    "",
    ...
    "",
    "A worker that genuinely does have the MCP may fetch its own spell instead; the bytes carry the same signed hashes either way, so the trust available is identical and only the failure mode differs — provided whatever fetches them verifies — and the datamancy MCP is that adapter, so for the worker described here it always does. Embedding is always correct. Fetching is correct only when you know it can succeed — which is why embedding is the default and the fallback both.",
    "",
