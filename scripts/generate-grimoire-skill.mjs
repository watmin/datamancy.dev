...
function render(spells) {
  const lines = [
    ...
    "",
    "**First establish whether your workers can reach this MCP at all.** You can; a spawned sub-agent often cannot — sandboxed, headless, no network, or simply launched without the server attached. **If they cannot, embedding is not a preference, it is the requirement:** you read the ward and paste it into their prompt. If they genuinely can reach it, they may fetch it themselves — the bytes carry the same signed hashes either way, so the trust available is identical; what differs is whether the fetch can fail, and whether the client verifies. Read the ward through whichever surface you have — `resources/read` on its `uri`, or **`fetch_spell` with its short name** (e.g. `{\"spell\": \"intueri\"}`); both return the same bytes, and the adapter SHA-256-verifies either against the signed manifest at fetch. The underlying path is **`/<name>/SKILL.md`, e.g. `/intueri/SKILL.md` — never `/grimoire/<name>/SKILL.md`**. Paste it **verbatim** into the sub-agent's prompt. The ward travels into the worker **by value**.",
    "",
  ...
  lines.push("");
  lines.push(
    "Every spell this server publishes is hash-listed — for the `datamancy` npm adapter to verify against — in the manifest at `/.well-known/mcp/manifest.json`, which is itself signed with **ECDSA P-256 over SHA-256** by a key held non-exportably in **AWS KMS** — the private key material never touches a disk and every signature is logged in CloudTrail. The matching public key is pinned in the [`datamancy`](https://www.npmjs.com/package/datamancy) npm package source, and it is the adapter that performs the verification: tampered content cannot reach an LLM through it. A client that fetches these paths without checking the manifest — `curl`, a browser, an MCP client that skips verification — gets whatever the origin served.",
  );
  lines.push("");
  lines.push(
