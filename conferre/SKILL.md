...
The spec is read from the page. The code side is **read from the current working tree at cast time — not recalled, not inferred, not read from a doc.** A doc that describes the code is another spec; comparing two specs finds nothing. "What the code ACTUALLY does" is a fact you establish by reading the live source, not a memory you carry in.

A divergence asserted from a stale spec, a retired form, a different branch, or the caster's own training is **phantom** — it is not a finding, and asserting it as one is itself the Level-1 lie conferre exists to catch. The caster who reports "the spec says X but the code does Y" without having read the Y is the silent drift, now wearing the auditor's robe.

Before comparing, conferre establishes the code side is **live**:

- **Branch and recency.** Is this the working tree the practitioner is actually on, or a stale checkout? A finding against `main` when the work lives a thousand commits ahead on a feature branch is a finding against a ghost.
- **Living code vs graveyards.** Retired forms, unmigrated fixtures, and mid-rebuild modules `grep` identically to live code. A form's *presence* in a file is not its *aliveness* — a retired form lingering in old fixtures is a tombstone, not a resident. When in doubt, the caster runs it, or asks who knows what is alive; absence of certainty is not evidence of agreement.

**Every reported divergence carries a current-tree `file:line` citation, or it is withdrawn.** No citation, no finding. This is the one rule that separates a real audit from a confident hallucination.
...
- Grounding: the current-working-tree `file:line` that establishes the code-side fact (per the grounding clause). A divergence without one is withdrawn, not reported.
