...
Where other spells check structure, mora checks **time discipline**: does this code WAIT via mechanism, or via the wire?
...
**Direct sleep calls (always flag):**
...
**Timeout-as-mechanism (always flag):**
- `recv_timeout(Duration::*)` — the duration is the mechanism; if you need a deadline, deliver it via a timer-arm in select
- `wait_timeout(Duration::*)` on Condvar (Condvar is itself banned in zero-mutex substrates; double violation)
...
**Test-time pseudo-syncs (flag with rationale check):**
...
- A timerfd-arm in select — time arriving as an fd-event; honest (and the SHAPE this spell prepares the ground for).
...
- `no-reactor` — consumer's runtime has no timerfd/select infrastructure yet (use sparingly; the rune retires when the reactor lands)
...
A consumer's eventual reactor will expose timerfd as a select-arm. After that lands, the honest "sleep" looks like:

```clojure
(let [timer (after 100ms)
      outcome (select' [data-rx timer])]
  (match outcome
    (:Data v)  (handle v)
...
- The golang `select { case <-time.After(d): ... }` pattern — the canonical reference shape for time-as-channel-event
